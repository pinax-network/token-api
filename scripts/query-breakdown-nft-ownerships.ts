...
const address = getArg('address', '0xb4d7b28e9a5e4c5e5f5b5f5b5f5b5f5b5f5b5f5b'); // Example holder
...
function replaceParams(sql: string): string {
    const tokenIdFilter = tokenId ? `'${tokenId}'` : '';
    return sql
        .replace(/\{db_nft:Identifier\}/g, `\`${dbNft}\``)
        .replace(/\{contract:Array\(String\)\}/g, `['${contract}']`)
        .replace(/\{address:Array\(String\)\}/g, `['${address}']`)
        .replace(/\{network:String\}/g, `'${network}'`)
        .replace(/\{token_id:Array\(String\)\}/g, tokenIdFilter ? `[${tokenIdFilter}]` : '[]')
        .replace(/\{token_standard:Nullable\(String\)\}/g, 'NULL')
        .replace(/\{include_null_balances:Bool\}/g, 'false')
...
async function main() {
    const queryDir = resolve(import.meta.dirname, '../queries/nft-query-breakdown/ownerships_evm');
    ...
    const fullSql = replaceParams(readFileSync(resolve(import.meta.dirname, '../src/routes/nft/ownerships_evm.sql'), 'utf-8'));
    ...
    console.log(`\n${'='.repeat(110)}`);
    console.log('QUERY BREAKDOWN: ownerships_evm.sql');
