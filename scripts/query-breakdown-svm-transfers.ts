...
// Usage: bun run scripts/query-breakdown-svm-transfers.ts [--network <net>] [--block <num>] [--mint <addr>]
...
const dbTransfers = networkConfig.transfers;

console.log(`Network: ${network}`);
console.log(`Cluster: ${networkConfig.cluster} (${cluster.url})`);
console.log(`Transfers DB: ${dbTransfers}`);
...
const queries: { name: string; sql: string }[] = [
    // blocks table lookups
    {
        ...
        sql: `SELECT timestamp FROM \`${dbTransfers}\`.blocks WHERE block_num = ${block}`,
    },
    {
        name: 'blocks: max block_num',
        sql: `SELECT max(block_num) AS latest FROM \`${dbTransfers}\`.blocks`,
    },

    // transfers: timestamp filters (primary index prefix)
    {
        name: 'transfers: WITH timestamp (last 1 min)',
        sql: `
            SELECT * FROM \`${dbTransfers}\`.transfers
    ...
    {
        name: 'transfers: WITH timestamp range',
        sql: `
            SELECT * FROM \`${dbTransfers}\`.transfers
    ...
    // transfers: timestamp + block_num (optimal)
    {
        name: 'transfers: WITH timestamp + block_num',
        sql: `
            SELECT * FROM \`${dbTransfers}\`.transfers
            WHERE timestamp >= now() - INTERVAL 1 MINUTE
              AND block_num >= ${block}
    ...
    // transfers: block_num only (FULL SCAN — the problematic case)
    {
        name: 'transfers: WITH block_num ONLY (⚠️ full scan)',
        sql: `
            SELECT * FROM \`${dbTransfers}\`.transfers
    ...
    // transfers: no filters (FULL SCAN)
    {
        name: 'transfers: NO filters (⚠️ full scan)',
        sql: `
            SELECT * FROM \`${dbTransfers}\`.transfers
            ORDER BY timestamp DESC
            LIMIT 10
    ...
    // transfers: block_num resolved via blocks table (THE FIX)
    {
        name: 'transfers: block→timestamp resolved (✅ fix)',
        sql: `
            WITH block_ts AS (
                SELECT maxOrNull(timestamp) AS ts FROM \`${dbTransfers}\`.blocks WHERE block_num = ${block}
            )
            SELECT * FROM \`${dbTransfers}\`.transfers
    ...
    // transfers: with mint filter
    {
        name: 'transfers: WITH mint filter',
        sql: `
            SELECT * FROM \`${dbTransfers}\`.transfers
            WHERE mint = '${mint}'
...
async function main() {
    ...
    console.log(`\n${'='.repeat(110)}`);
    console.log('SVM TRANSFERS — QUERY BREAKDOWN');
