...
WHERE
    ...
    (symbol0 in ('WBNB', 'WETH', 'USDC', 'USDT', 'DAI', 'BUSD') OR symbol1 in ('WBNB', 'WETH', 'USDC', 'USDT', 'DAI', 'BUSD')) AND
    if (symbol0 in ('WBNB', 'WETH'), reserve1 > 0.05, true) AND -- $100 worth of Native
    if (symbol1 in ('WBNB', 'WETH'), reserve1 > 0.05, true) AND -- $100 worth of Native
    if (symbol0 in ('USDC', 'USDT', 'DAI', 'BUSD'), reserve0 > 100.0, true) AND -- $100 worth of Stable
    if (symbol1 in ('USDC', 'USDT', 'DAI', 'BUSD'), reserve1 > 100.0, true) AND -- $100 worth of Stable
