WITH erc721 AS (
    ...
    FROM (
        SELECT
            ...
            owner,
        FROM erc721_owners
        WHERE owner IN {address:Array(String)} AND ({contract: String} = [''] OR contract IN {contract:Array(String)})
    ) AS o
...
erc1155 AS (
    ...
    FROM (
        ...
        WHERE owner IN {address:Array(String)} AND ({contract: String} = [''] OR contract IN {contract:Array(String)}) AND balance > 0
    ) AS o
