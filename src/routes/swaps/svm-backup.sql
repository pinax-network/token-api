...
(
    ...
    WHERE
        ...
        AND (isNull({start_block:Nullable(UInt32)}) OR timestamp >= (SELECT timestamp FROM {db_dex:Identifier}.blocks WHERE block_num >= {start_block:Nullable(UInt32)} ORDER BY block_num ASC LIMIT 1))
